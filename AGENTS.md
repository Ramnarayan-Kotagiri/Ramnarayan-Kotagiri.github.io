...
- Serve locally with `npx serve .`; there is no build step.
- GitHub Pages publishes the root of `master` at `/`.
- Keep structured projects, publications, capabilities, and twin demo values in `data.js`.
- Treat the oil palm application as closed-source. Never expose its repository, code, private content, endpoints, or unapproved screenshots.
- Label conceptual interfaces, future architecture, illustrative outputs, and simulated telemetry explicitly.
